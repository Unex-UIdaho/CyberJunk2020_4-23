...
public class PlayerScript : MonoBehaviour
{
    ...
    public List<GameObject> weaponInventory; //This contains the weapon object
    ...
    void Start()
    {
        //Create gun
        weaponIndex = 0;
        weapon = Instantiate(weaponInventory[weaponIndex], transform.position, Quaternion.identity) as GameObject;
        weaponScript = weapon.GetComponent<GunScript>();
        weaponScript.initialDelay = weaponScript.automaticDelay;
        weaponScript.initialAutomaticAssault = weaponScript.automaticAssault;
        ...
        //Assign Currency Manager Component
        moneyCounter = GetComponent<CurrencyManager>();
    }
        ...
        if (Time.timeScale != 0)
        {
            ...
            if (!Global.shopActive)
            {
                ...
                if (weapon)
                {
                    ...
                    if (movementy > Mathf.Abs(movementx))
                    {
                        sort.sortingOrder = selfsort.sortingOrder - 10;
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        if(col.gameObject.tag == "Coin") //player portion of loot pickup, calls member function of CoinPickup.cs class
        {
            CoinPickup coin = col.transform.GetComponent<CoinPickup>();
            coin.Collected();
            moneyCounter.availableMoney++;
        }
    }
