...
public class EnemyScript : MonoBehaviour
{
    ...
    //gameObject to instantiate loot drop on enemy death
    ...
    void Update()
    {
        ...
        {
            GetComponent<AIPath>().destination = patrolQueue[patrolIndex];
            ...
            if (Vector2.Distance(transform.position, patrolQueue[patrolIndex]) < 0.2f)
            {
                patrolIndex++;

                if (patrolIndex > patrolQueue.Length - 1)
                {
                    patrolIndex = 0;
                }
            }
        ...
        if (health <= 0)
        {
            ...
            Destroy(gameObject);
            coinsDropped = Random.Range(0,4);
            for(int i = 0; i < coinsDropped; i++)
            {
                Instantiate(lootDrop, transform.position, Quaternion.identity); //instantiate between 1 and 5 coins for looting
            }
        ...
        if (weapon)
        {
            ...
            weaponScript.flashlightOn = true;
    ...
    void FixedUpdate()
    {
        ...
        if (player)
        {
            ...
            float movementAngle = Mathf.Abs(Mathf.Atan2(movementy, movementx));

            float playerAngle = Mathf.Abs(Mathf.Atan2(player.transform.position.y - transform.position.y, player.transform.position.x - transform.position.x));

            if (playerAngle < (movementAngle + 0.7f) && playerAngle > (movementAngle - 0.7f))
            { }
            else if (Vector2.Distance(transform.position, player.transform.position) > 2)
                shoot = false;

            //Debug.Log("Movement Angle: " + movementAngle + ", playerAngle: " + playerAngle);
